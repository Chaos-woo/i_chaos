...
class OptionBarList extends StatelessWidget {
  ...
  OptionBarList({
    ...
    TextStyle tipTextStyle = const TextStyle(
      fontSize: 12, /*fontWeight: FontWeight.w300, color: Colors.black54*/
    ),
    double optionItemHeight = 45,
      ...
      itemBuilder: (ctx, index) {
        ...
        for (int i = 0; i < currentItemGroupLength; i++) {
          if (i != 0) {
            ...
            widgetOptionItems.add(_widgetOptionItem(optionItemGroup[i], theme));
          } else {
            widgetOptionItems.add(_widgetOptionItem(optionItemGroup[i], theme));
          }
        }
  ...
  // 选项组件
  Widget _widgetOptionItem(OptionBarItem item, ThemeData theme) {
        ...
        if (item.icon != null)
          Padding(
            ...
            child: Icon(
              item.icon,
              size: iconSize,
              color: item.iconColor ?? theme.primaryColor,
            ),
          ),
        Text(
          item.text,
      ...
      children: [
        ...
        if (item.rightIcon != null)
          Icon(
            item.rightIcon,
            size: rightIconSize,
            color: item.rightIconColor ?? theme.primaryColor,
          ),
      ],
    );
    ...
    if (item.extPosition == p) {
      if (item.extensionWidget != null) {
        ...
        extension.add(
          Text(
            item.extensionText!,
            style: item.textStyle!.merge(const TextStyle(fontSize: 13)),
          ),
        );
      }
    }
