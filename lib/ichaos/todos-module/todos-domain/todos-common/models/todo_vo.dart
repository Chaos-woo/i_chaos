...
import 'package:i_chaos/base-getX-framework/utils/random.dart';
...
class TodoVO extends MultiSortBase with SerializeVO<TodoEntity> {
  ...
  @override
  TodoEntity toEntity() {
    return TodoEntity(level, completed, period,
        updateTime: updateTime.yyyyMMddHHmmss,
        id: id,
        content: content,
  ...
  /// 卡片区域信息分布
  bool get isColorPrompt => level > TodoLevel.third.code;
  ...
  // bool get isTagsPrompt =>
  //     level > TodoLevel.normal.code || period > 0 || (tag != null && tag! > 0);
  ...
  static TodoVO fakeTester() {
    ...
    TodoVO vo = TodoVO.newTodo(
        content: 'fake testing',
        subTaskList: tasks,
        remark: 'fake remark',
