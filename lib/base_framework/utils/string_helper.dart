...
class StringHelper {
  ...
  static Widget getSlashPrice(String price, double markSp, double sizeSp,
    ...
    return Row(
      children: <Widget>[
        ...
        Text(
          price,
          style: TextStyle(color: priceColor, decoration: TextDecoration.lineThrough, decorationColor: priceColor, fontSize: sizeSp),
        ),
      ],
    );
  }
  ...
  ///不带钱符号的驼峰价格
  static Widget getCamelPriceWithoutSymbol(String price, double intSp, double decimalSp,
      {Color priceColor = Colors.black}) {
    return Text.rich(TextSpan(
      children: [
        TextSpan(
          text: getDoublePriceInt(double.parse(price)),
          style: TextStyle(color: priceColor, fontSize: intSp, fontWeight: FontWeight.bold),
        ),
        TextSpan(
          text: ".",
          style: TextStyle(color: priceColor, fontSize: decimalSp, fontWeight: FontWeight.bold),
        ),
        TextSpan(
  ...
  static Widget getPriceWithoutCamel(String price, double markSp, double priceSp,
    ...
    return Text.rich(TextSpan(children: [
      TextSpan(
        text: moneySymbol,
        style: TextStyle(color: symbolColor ?? priceColor, fontSize: markSp, fontWeight: fontWeight),
      ),
      TextSpan(
  ...
  static Widget getUsaStylePrice(
    ...
    return Text.rich(TextSpan(children: [
      TextSpan(
        text: symbol,
        style: TextStyle(color: symbolColor ?? priceColor, fontSize: symbolSize, fontWeight: symbolWeight),
      ),
      TextSpan(
  ...
  static String getDoublePriceDecimal(double price) {
    return price > 0 ? ((price - price.truncate()).toStringAsFixed(2)).substring(2) : "00";
  }

  static int paresStringColor(String color) {
    if (color.isEmpty) return 0xFF222222;
