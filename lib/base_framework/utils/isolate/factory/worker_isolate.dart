...
import 'package:i_chaos/base_framework/utils/isolate/factory/task_warpper.dart';
import 'package:i_chaos/base_framework/utils/isolate/factory/work_isolate_wrapper.dart';
...
void proxyHandler(SendPort mainPort) async {
  ...
  List.generate(3, (index) async {
    ...
    Isolate.spawn(_workerIsolate, proxySendPort, paused: true).then((isolate) {
      ...
      workers.forEach((key, value) {
        if (taskLog.length > 0) {
          if (value.isStandBy()) {
            ...
            value.workSendPort!.send([
              kTaskKey,
              {kMethodName: task.methodName, kNameArgs: task.nameArgs}
            ]);
  ...
  receivePort.listen((message) {
    if (message[0] == kTaskKey) {
      ...
      if (method[kNameArgs] is Map) {
        ...
        proxyPort.send([
          kWorkDone,
          {'method': mn, 'args': nameArguments.toString()}
        ]);
      }
    }
  });
